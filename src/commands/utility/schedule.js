...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'add') {
      ...
      if (!delay || delay < 30_000) return interaction.reply({ embeds: [Embed.error('The delay must be at least 30 seconds.')], ephemeral: true });
      const repeatStr = interaction.options.getString('repeat');
      let interval = 0;
      if (repeatStr) {
        interval = parseDuration(repeatStr);
        if (!interval || interval < 600_000) return interaction.reply({ embeds: [Embed.error('Repeat interval must be at least 10 minutes.')], ephemeral: true });
      }
      const perms = channel.permissionsFor(interaction.guild.members.me);
      if (!perms?.has(PermissionFlagsBits.SendMessages)) return interaction.reply({ embeds: [Embed.error(`I cannot send messages in ${channel}.`)], ephemeral: true });
      const nextRun = Date.now() + delay;
      const info = insert.run(gid, channel.id, message, nextRun, interval, interaction.user.id, Date.now());
      return interaction.reply({ embeds: [Embed.success(`Scheduled message **#${info.lastInsertRowid}** for ${channel} — first send ${relative(nextRun)}${interval ? `, then every **${formatDuration(interval)}**` : ' (one-time)'}.`)] });
    }

    if (sub === 'list') {
      const rows = list.all(gid);
      if (!rows.length) return interaction.reply({ embeds: [Embed.info('🗓️ Scheduled Messages', 'Nothing scheduled.')] });
      const embed = new EmbedBuilder().setColor(config.brand.color).setTitle('🗓️ Scheduled Messages')
    ...
    if (!getOne.get(id, gid)) return interaction.reply({ embeds: [Embed.error('No scheduled message with that ID.')], ephemeral: true });
    del.run(id, gid);
    return interaction.reply({ embeds: [Embed.success(`Deleted scheduled message #${id}.`)] });
  },
};
