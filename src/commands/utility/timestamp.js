...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (offset && ms === null) return interaction.reply({ embeds: [Embed.error(t(gid, 'util.timestamp.invalid'))], ephemeral: true });
    const ts = Math.floor((Date.now() + (ms || 0)) / 1000);
    const formats = ['t', 'T', 'd', 'D', 'f', 'F', 'R'];
    const embed = new EmbedBuilder().setColor(config.brand.color).setTitle(t(gid, 'util.timestamp.title'))
